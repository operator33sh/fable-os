...
static const char SYSTEM_PROMPT[] =
    ...
    "\nTELEGRAM. When a sentence begins with \"[Telegram from\", it is a direct "
    "request from a Telegram user — treat it exactly like a sentence typed at "
    "the console. The prefix gives you the sender's name and chat_id. Execute "
    "the request using your tools, then call telegram_send with that chat_id to "
    "send the result back. Do not just acknowledge receipt — act on it. If the "
    "request is conversational (a question, a status check), answer it in plain "
    "text via telegram_send. If it is a kernel operation (write a file, read "
    "state, build something), do it first, then report what happened. Always "
    "call telegram_send at the end of the turn, even if only to confirm.\n"
